...
// --- 작성자 본인이 자기 글에 이어서 단 댓글(체이닝 답글)을 텍스트로 추출 ---
// og:description과 달리 공식 메타데이터가 아니라 실제 DOM을 뒤져서 찾는 방식이라
// 사이트 구조가 바뀌면 깨질 수 있는 최선 노력(best-effort) 추출이다.
async function extractAuthorReplies(page, mainCaption) {
  try {
    return await page.evaluate((caption) => {
      const normalize = (s) => (s || '').replace(/\s+/g, ' ').trim();

      const match = location.pathname.match(/^\/@([^/]+)\//);
      if (!match) return [];
      const linkPrefix = `/@${match[1]}/post/`;

      const cards = new Set();
      document.querySelectorAll(`a[href^="${linkPrefix}"]`).forEach((anchor) => {
        let el = anchor;
      ...
      const seen = new Set([normalize(caption)]);
      const replies = [];
      cards.forEach((card) => {
...
async function captureScreenshot(rawUrl) {
  ...
  try {
    ...
    const authorReplies = await retryOnNavigation(() => extractAuthorReplies(page, caption));
