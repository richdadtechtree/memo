...
// 브라우저에서 내보낸 로그인 세션(scripts/cookies-to-storage-state.js로 생성).
// 있으면 로그인 상태로 캡처해 스레드의 비로그인 방문자 제한(로그인 벽)을 피한다.
const STORAGE_STATE_PATH = path.join(__dirname, '..', 'auth', 'threads-storage-state.json');
...
async function installOverlayBlocker(page) {
  await page.addInitScript(() => {
    ...
    // 팝업 본체를 지워도, 화면을 어둡게 덮는 배경(백드롭)만 형제 요소로 남는 경우가 있다.
    // 글자가 없어 키워드로는 못 잡으니, 화면 전체를 덮는 반투명/불투명 고정 요소를 따로 찾는다.
    function isFullscreenBackdrop(el) {
    ...
    function sweep() {
      ...
      // 태그 종류에 상관없이, 화면에 고정 배치된 요소만 먼저 추린 뒤 검사한다.
...
// --- Esc 키 입력 (보조 수단) ---
// 실제 오버레이 제거는 installOverlayBlocker의 MutationObserver가 계속 감시하며
// 처리하므로, 여기서는 Esc 키만 눌러 그것으로 닫히는 팝업을 보조로 처리한다.
